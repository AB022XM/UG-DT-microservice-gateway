...
import ug.co.absa.gateway.config.KafkaSseConsumer;
import ug.co.absa.gateway.config.KafkaSseProducer;

@RestController
@RequestMapping("/api/gateway-service-ug-kafka")
public class GatewayServiceUgKafkaResource {

    private final Logger log = LoggerFactory.getLogger(GatewayServiceUgKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public GatewayServiceUgKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
