...
import ug.co.absa.gateway.config.EmbeddedKafka;
...
@WithMockUser
@EmbeddedKafka
class GatewayServiceUgKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/gateway-service-ug-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/gateway-service-ug-kafka/consume")
